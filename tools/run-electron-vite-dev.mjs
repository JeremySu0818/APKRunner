...
if (electronFlags.length > 0) {
  const electronPath = join(repoRoot, "node_modules", "electron", "dist", "electron");
  const wrapperPath = join(repoRoot, "node_modules", ".cache", "apkrunner", "electron-dev");
...
function parseElectronFlags(value) {
  ...
  try {
    ...
    // Fall through to whitespace parsing for simple shell-friendly values.
  }
...
function getElectronFlags(wslg) {
  const configuredFlags = parseElectronFlags(process.env.APKRUNNER_ELECTRON_FLAGS);
  if (configuredFlags.length > 0) {
    return configuredFlags;
  }

  return wslg ? ["--no-sandbox", "--in-process-gpu", "--disable-gpu-compositing"] : [];
}

function isWslg() {
  return process.platform === "linux" && Boolean(process.env.WSL_DISTRO_NAME) && Boolean(process.env.WAYLAND_DISPLAY);
}
